...
jest.mock('@/src/components/UserSearch', () => ({ UserSearch: () => null }));
jest.mock('@/src/components/NewChatModal', () => ({ NewChatModal: () => null }));
jest.mock('@/src/hooks/useFacebookInvite', () => ({
  useFacebookInvite: () => ({ invite: jest.fn(), isLoading: false }),
}));
jest.mock('axios');

import { AuthProvider } from '@/src/context/AuthContext';
import { ThemeProvider } from '@/src/context/ThemeContext';
import { FriendsListPage } from '@/src/pages/friends/FriendsListPage';
...
describe('FriendsListPage', () => {
  it('renders without crashing', () => {
    expect(() => render(<FriendsListPage />, { wrapper: Wrapper })).not.toThrow();
  });
});
